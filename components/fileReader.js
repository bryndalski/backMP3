            ...
            fs.readdir(path.join(dirname, directory), function (err, files) {
                ...
                new Promise((suc, er) => {
                    ...
                    let recursiveFunction = function () {
                        data[files[counter]] = []
                        fs.readdir(path.join(dirname, path.join(directory, files[counter])), async function (err, insidefiles) {
                            if (err)
                                return console.log(err);

                            insidefiles.forEach(index => {
                                console.log(index)
                                let fileStats = fs.statSync(path.join(dirname, `${directory}${files[counter]}/${index}`))
                                data.push({
                                    source: `${directory}${files[counter]}/${index}`,
                                    name: index,
                                    size: Math.round((fileStats.size / (1024 * 1024) + Number.EPSILON) * 100) / 100,
                                    path: files[counter]
                                })
                            })
                            counter++
